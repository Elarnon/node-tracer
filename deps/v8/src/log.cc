...
bool NewAddress(SnapshotObjectId addr, bool makeNew = false) {
  ...
  if (i == processed.end()) {
    // process the address
    if (!makeNew) processed.insert(addr);
    return true;
  } else {
    if (makeNew) processed.erase(i);
    return false;
  }
}
...
/** Converts given object into a unique id that is used in the log reports. If
 * the object is not HeapObject, returns 0.
 */
SnapshotObjectId HeapObjectToId(Handle<HeapObject> obj,
                                bool& isFirstOccurence) {
  // Isolate::Current()->heap()->AddHeapObjectAllocationTracker(new
  // MyHeapTracker());
  HeapProfiler* hp = Isolate::Current()->heap_profiler();
  if (!hp->is_tracking_object_moves()) {
    std::ostringstream s;
    s << ((size_t)hp);
    LOG(Isolate::Current(),
        StringEvent("heap-tracking-start", s.str().c_str()));
    hp->StartHeapObjectsTracking(/* trackAllocations = */ true);
  }
  SnapshotObjectId id =
      hp->heap_object_map()->FindEntry(Address(obj->address()));
  if (id != 0) {
    isFirstOccurence = false;
  } else {
    isFirstOccurence = true;
    id = hp->heap_object_map()->FindOrAddEntry(Address(obj->address()),
                                               obj->Size());
  }
  return id;
}
...
SnapshotObjectId MapToId(Map* map, bool& isFirstOccurence) {
  ...
  /*        if (! hp->is_tracking_object_moves()) {
      std::ostringstream s;
      s << ((size_t) hp);
      LOG(Isolate::Current(), StringEvent("heap-tracking-start",
  s.str().c_str())); hp->StartHeapObjectsTracking(/ * trackAllocations = * /
  false);
  } */
  SnapshotObjectId id =
      hp->heap_object_map()->FindEntry(Address(map->address()));
  if (id != 0) {
    isFirstOccurence = false;
  } else {
    isFirstOccurence = true;
...
void PrintLocation(Log::MessageBuilder& msg) {
  ...
  for (StackTraceFrameIterator it(i); !it.done(); it.Advance()) {
    ...
    msg << *Object::NoSideEffectsToString(i, filename) << ":"
        << site.GetLineNumber();
    return;
  }
  msg << "native:0";
...
void Logger::MapChangeEvent(Handle<HeapObject> object, SnapshotObjectId id,
...
void Logger::GetPropertyEvent(Handle<Object> target, Handle<Name> key) {
  ...
  if (NewAddress(mapId)) MapChangeEvent(obj, id, mapId);
  Log::MessageBuilder msg(log_);
  msg << "get-property" << kNext << id << kNext << mapId << kNext << *key
      << kNext;
  PrintLocation(msg);
  msg.WriteToLogFile();
}
...
void Logger::GetElementEvent(Handle<Object> target, uint32_t index) {
  ...
  if (NewAddress(mapId)) MapChangeEvent(obj, id, mapId);
  Log::MessageBuilder msg(log_);
  msg << "get-element" << kNext << id << kNext << mapId << kNext << index
      << kNext;
  PrintLocation(msg);
  msg.WriteToLogFile();
}
...
void Logger::SetPropertyEvent(Handle<Object> target, Handle<Name> key,
  ...
  if (NewAddress(mapId)) MapChangeEvent(obj, id, mapId);
  Log::MessageBuilder msg(log_);
  msg << "set-property" << kNext << id << kNext << mapId << kNext << *key
      << kNext;
  if (value->IsSmi()) {
    msg << "smi(";
...
void Logger::SetElementEvent(Handle<Object> target, uint32_t index,
  ...
  if (NewAddress(mapId)) MapChangeEvent(obj, id, mapId);
  Log::MessageBuilder msg(log_);
  msg << "set-element" << kNext << id << kNext << mapId << kNext << index
      << kNext;
  if (value->IsSmi()) {
    msg << "smi(";
    std::ostringstream os;
...
void Logger::AllocationEvent(Address addr, size_t size) {
  if (FLAG_predictable || !log_->IsEnabled()) return;
